...
machineForTopFunction :: StackMachineSync -> (StackMachineSync, StackMachineSync)
machineForTopFunction state =
  let topBody = extractCurrentBody (state ^. bytecodeSync)
      remainingBytecode = dropCurrentBody (state ^. bytecodeSync)
      forkedMachine = state & bytecodeSync .~ topBody
      currentMachine = state & bytecodeSync .~ remainingBytecode
   in (forkedMachine, currentMachine)

machinesForTopTwoFunctions :: StackMachineSync -> (StackMachineSync, StackMachineSync, StackMachineSync)
machinesForTopTwoFunctions state =
  let (forked1, current1) = machineForTopFunction state
      (forked2, current2) = machineForTopFunction current1
   in (forked1, forked2, current2)
